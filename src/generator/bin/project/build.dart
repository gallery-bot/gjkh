...
Future prepareProject(Project project) async {
  ...
  if (!File('${projectPath}web/index.html').existsSync()) {
    final progress =
        logger.progressSection('Adding web support for flutter project');
    final getPackages = await Process.start('flutter', ['create', '.'],
  ...
  final getPackages = await Process.start('flutter', ['pub', 'get'],
      workingDirectory: projectPath, runInShell: true);
...
Future moveWebProject(Project project) async {
  ...
  final moveProjectProgress =
      logger.progressSection('Move web project to $webPath');
